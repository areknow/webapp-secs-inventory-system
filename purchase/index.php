...
<html>
    <head>
        ...
        <title>SECS Inventory | Purchase</title>
    ...
    <body>
		...
		<div class="nav">
			...
			<div class="sidemenu">
				...
				<a href="../purchase" class="selected">Purchase Info</a>
		...
		<div class="content">
			<table id="computers" class="display" cellspacing="0" width="100%">
				...
				<tbody>
				<?PHP $query = mysql_query("SELECT * FROM `Purchase Info`");
				while ($results = mysql_fetch_array($query)) { ?>
					<tr>
						<td><?PHP echo $results['Tag'] ?></td>
						<td><?PHP echo $results['Original PO'] ?></td>
						<td><?PHP echo $results['Vendor'] ?></td>
						<td><?PHP echo $results['Cost'] ?></td>
						<td><?PHP echo $results['Account'] ?></td>
						<td><?PHP echo $results['Acq Date'] ?></td>
						<td><?PHP echo $results['Warranty Expiration'] ?></td>
					</tr>
				<?PHP } ?>
				</tbody>
			</table>
